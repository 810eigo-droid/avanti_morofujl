...
(function(){var header=document.querySelector('.site-header');var button=document.querySelector('.menu-toggle');if(!header||!button)return;function close(){header.classList.remove('menu-open');button.setAttribute('aria-expanded','false');button.setAttribute('aria-label','メニューを開く')}button.addEventListener('click',function(){var open=header.classList.toggle('menu-open');button.setAttribute('aria-expanded',String(open));button.setAttribute('aria-label',open?'メニューを閉じる':'メニューを開く')});header.querySelectorAll('nav a').forEach(function(link){link.addEventListener('click',close)})})();`,
        ...
        />
        <script dangerouslySetInnerHTML={{ __html: "(function(){var v=document.querySelector('.menu-gallery-window');if(!v||innerWidth>800)return;setTimeout(function(){var sample=v.scrollLeft;setTimeout(function(){if(Math.abs(v.scrollLeft-sample)>2)return;v.classList.add('is-auto-moving');var last=0,pauseUntil=0,cycle=v.scrollWidth/2;function pause(){pauseUntil=performance.now()+4500;last=0}function move(t){if(t>=pauseUntil&&!document.hidden){if(last)v.scrollLeft+=(t-last)*.018;if(v.scrollLeft>=cycle)v.scrollLeft-=cycle;last=t}else last=0;requestAnimationFrame(move)}v.addEventListener('pointerdown',pause,{passive:true});v.addEventListener('touchstart',pause,{passive:true});requestAnimationFrame(move)},1200)},600)})();" }} />
      </body>
    </html>
  );
}
